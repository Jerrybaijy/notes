...
Shell 花括号扩展是 Bash/Zsh 等主流 Shell 内置的**字符串生成机制**，核心作用是通过简洁语法批量生成多个字符串（常用来创建目录、文件，或构造命令参数），无需重复输入重复部分，效率极高。它是纯 Shell 解析（不调用外部命令），执行速度快，且支持嵌套、范围扩展等灵活用法。
...
# 文件：生成 src/pages/Home.js、src/pages/About.js、src/components/Button.js
touch src/{pages/{Home,About},components/Button}.js
```
