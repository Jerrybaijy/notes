...
- 根据提示在 SDK Shell 中选择并在浏览器中登录 Google
- 使用本地 SDK Shell 代替网页版的 Cloud Shell
- 安装 `gke-gcloud-auth-plugin` 插件，否则无法使用 `kubectl` 命令来管理您的集群
...
- 按照以下步骤为 Cloud Interconnect 连接创建 VLAN 连接
...
- 如果安装便携版安装之后需手动加入开始菜单，详见 `Windows` > `加入开始菜单`。
- 先建立本地音乐库，并持续监控。
...
- 以 Git Bash 为例（Git Bash 可以在安装时勾选添加到 Windows Terminal）
- 电脑中安装好 Git
