...
> [我要自学网：燎原《Python 编程入门 2021 新版教程》](https://www.51zxw.net/List.aspx?cid=929)
>
> [B 站：银角大王-武沛齐《精品 Python 全套教程》](https://www.bilibili.com/video/BV1zd4y1y7px/?spm_id_from=333.337.search-card.all.click&vd_source=02dd98cfbf740b256dca0ec3e66776e6)
...
- 如果你需要安装特定版本的 Python（比如 Python 3.11），你可以使用：
...
- [官网下载社区免费版 `PyCharm Community Edition`](https://www.jetbrains.com/pycharm/download/?section=windows)
- 勾选如图 2 项
...
- [官网下载商业版 `Pycharm Professional`](https://www.jetbrains.com/pycharm/download/?section=windows)；
- 默认安装，安装完成后不要立即运行；
- 以下破解方法来源于 [`小墨の博客`](https://www.only4.work/blog/?id=473)；
- 博客地址或网盘下载并解压激活工具 `【请解压】JetBrains 2023.1 永久激活插件`；
- 为了避免路径问题，请将解压后的文件夹到本地一个没有中文以及特殊符号的路径下，且配置后不能移动和删除；
- `jetbra` > `scripts` > 双击运行 `install-all-users.vbs` 脚本 > 出现 `done` 弹框，说明成功；

  <img src="assets/image-20231026232300636.png" alt="图片1" style="width: 50%;" />

- 由于存在环境变量配置，插件安装之后，需要重启电脑才会生效。
- 重启后，在博客内获取激活码；
- 最后一次获取到的是（本次激活有效期至 2025.08.）：
...
- 此时运行 PyCharm Professional，在 `activation code` 选项下，填入最新激活码，点击 `Activate` 就能完成激活了；
...
- **缩进**
  - `Main menu` > `Settings` > `Editor` > `Code Style` > `Python`
  - 设置 `Tab size` `Indent` `Continuation indent` 三项为 `2`

- **鼠标滚轮**
  - `Main menu` > `Settings` > `Editor` > `General` > `Mouse Control` 选项
  - `Change font size with Ctrl + Mouse Wheel` 打上对勾
  - `All editors` 打勾

- **设置代码字体大小**
  - `Main menu` > `Settings` > `Editor` > `Font`
  - `Size = 24`，`Line Height = 1.2`
  - 如果设置不生效，`Main menu` > `Settings` > `Editor` > `Color Scheme` > `Color Scheme Font` > 取消勾选 `Use color scheme font instead of the default`
...
- **代码格式化**
  ...
  - 编辑完代码按 `Ctrl + Shift + L`，可实现自动更正空格格式。注意注释部分不会被更正

- **安装主题**：`主菜单` > `设置` > `外观与行为` > `外观` > `获取更多主题`
- **创建文件**
  - `项目路径右键` > `New` > `Python Fire` > 给文件命名 > `Enter`；
  - 编写代码，自动保存；
  - 运行：（1、项目栏，2、右键，3、快捷键）快捷键：`Ctrl + Shift + F10`。

- **同一项目添加多个文件夹**
  - `Settings` > `Project` > `Project Structure`
  - 右侧 `Add Content Root` > `选择要添加的文件夹` > `OK`

- **将 Terminal 默认项切换至 cmd**
  - Terminal 可以模拟 cmd
  - `Settings` > `Tools` > `Terminal` > `Shell path` 下拉菜单，`切换至 cmd`

- **无法调用第三方模块**
  - `Settings` > `Project：当前文件名` > `Python Interpreter` > `右侧Add Interpreter` > `Existing` > `右侧…`
  - 找到 `python.exe/` 将 `python.exe` 拖入窗口 > `OK`
...
- IDE 安装 `Black Formatter` 扩展

- IDE `settings.json` 设置

    ```json
    {
      "[python]": {
        "editor.tabSize": 4, // 默认 1 个 Tab 的宽度为 4 个空格
        "editor.defaultFormatter": "ms-python.black-formatter", // Python 默认格式化工具
      }
    }
    ```
...
- **普通字符串**：Python 中也可使用单引号 `'` 标记普通字符串。
- **子字符串**：Python 中也可使用双引号 `"` 和 单引号 `'` 嵌套表示子字符串。
- **多行字符串**：Python 中也可使用**模板字符串**或**三重单引号 `'''`** 表示多行字符串
...
- 一张纸的厚度大约是 0.08mm，对折多少次之后能达到珠穆朗玛峰的高度（8848 米）?
...
- Python 中有 `if` 和 `三元表达式` 两种选择结构，作用同 Java，格式不同。
...
- **扩展**：由于在 Python 中其它数据类型可以隐式转换成布尔型，所以 if 语句可以写成如下：

  ```python
  if condition:  # 如果 condition 的布尔值为 true，则执行
      执行语句
  ```
...
- Python 中支持**无限循环**和**循环嵌套**，作用同 Java，格式不同。
...
- Python 中，`for-in` 循环用于遍历**可迭代对象**（如字符串、列表、元组、字典和集合等），可迭代对象有几个元素，就会执行几次循环体，每次循环时从中取出的一个元素，将赋值给该变量。
- **语法**
...
- Python 中有 `continue`、`break`、`return` 和 `raise` 四种跳转结构。
- 其中 `continue`、`break`、`return` 用法同 Java。
...
- **id() 获取 id**：获取一个数据的 id，判断两个变量是不是同一个变量
...
- `__file__`：**file**指的是脚本所在的路径，有可能是相对路径
- **获取当前文件绝对路径**
...
- 位置 1，推荐
  - 确保 app01 已注册
  - Django 去 settings.py 文件中 INSTALLED_APPS（已注册的所有 app）依次寻找 templates 模板
  - 在 app01 里创建 templates 文件夹，里面创建名为 home 的 html 文件

- 位置 2，不推荐
  - Django 优先去 settings.py 文件中 TEMPLATES -> 'DIRS': [], 寻找 templates 模板
  - 在项目根目录创建 templates 文件夹，里面创建名为 home 的 html 文件
...
- 位置 1 推荐
  - 确保 app 已注册
  - Django 去 settings.py 文件中 INSTALLED_APPS（已注册的所有 app）依次寻找 templates 模板
  - 在 app01 里创建 templates 文件夹，里面创建名为 static 的文件夹
  - static 里创建如下文件夹
...
- 引入方式
  ...
  **在以上代码中**：
  - 在顶部引入 static 文件夹确保系统能找到路径，输入路径时才能有提示。
  - 引入路径时 `src="{% static '...' %}"`
  - 定义 class 方式不变
...
- 创建表
  ...
  - 方法 2
    - Pycharm --> 工具 --> 运行 manage.py 任务（Ctrl+Alt+R）
    - 先后输入执行 makemigrations 和 migrate
