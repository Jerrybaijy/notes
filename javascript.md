...
[`Map.keys()`](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Map/keys) 方法用于迭代 `Map` 对象中的所有键，返回一个新的[_迭代器_](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Iterator)对象，键按插入顺序排列。
...
**Canvas 劫持**（Canvas Hijacking/Interception）是一种绕过前端绘图限制、直接获取渲染数据的技术。

通常，当网站为了防爬虫或保护隐私，选择不使用传统的 HTML 标签（如 `<div>` 或 `<span>`）来展示文本，而是将信息直接绘制在 `<canvas>` 画布上时，普通的 DOM 抓取工具（如 Puppeteer）就会失效。

## 前提条件

需要在网页加载时将“注入脚本”注入到浏览器，然后通过“注入脚本”将“沙盒脚本”注入到浏览器**原生环境**。
...
(function () {
  ...
  const BRIDGE_EVENT_NAME = "__hunter_rpa_bridge__";

  function createBridge() {
    const handlers = new Map();
    const send = (type, payload) => {
      const event = new CustomEvent(BRIDGE_EVENT_NAME, {
        detail: { type, payload, _from: "sandbox" }
      });
    ...
    return {
      ...
      listen: () => {
        document.addEventListener(BRIDGE_EVENT_NAME, (event) => {
          ...
          if (!data || !data.type || data._from === "sandbox") return;
  ...
  // 定义被劫持内容的容器
  const capturedPoints = new Map();

  function formatResumeContent() {
    const points = Array.from(capturedPoints.values());
    console.log("[Hunter Sandbox] Formatting total points:", points.length);
    ...
    for (const p of points) {
      if (Math.abs(p.y - lastY) < 1 && Math.abs(p.x - lastX) < 1) continue;

      if (Math.abs(p.y - lastY) >= 8) {
        result += "\n";
        lastX = -Infinity;
      } else if (p.x - lastX > 15) {
        result += "  ";
      }
      
      result += p.text;
      lastX = p.x + (p.text.length * 10);
      lastY = p.y;
    }
    ...
    let fullText = result.split("为妥善保护")[0].split("您可以直接")[0];
    for (const [tag, label] of sections) {
      fullText = fullText.replaceAll(tag, label);
    }
  ...
  function initHijack() {
    ...
    bridge.on("__get_boss_canvas_resume_content_1__", (payload) => {
      const content = formatResumeContent();
      bridge.send("__get_boss_canvas_resume_content_2__", {
        content,
        resumePlatformKey: payload.resumePlatformKey
      });
    });

    bridge.on("CLEAR_CAPTURE_CACHE", () => {
      capturedPoints.clear();
      console.log("[Hunter Sandbox] Capture cache cleared.");
    });
    ...
    try {
      ...
      // 存储被劫持的 context 对象
      const hijackedContexts = new WeakSet();

      // 修改原始 getContext 方法
      HTMLCanvasElement.prototype.getContext = function (type, options) {
        // 调用原始方法获取 2D 绘图上下文 对象
        const context = originalGetContext.call(this, type, options);
        if (type === "2d" && context && !hijackedContexts.has(context)) {
          hijackedContexts.add(context);
          ...
          context.fillText = function (text, x, y, maxWidth) {
            if (typeof text === "string" && text.trim().length > 0) {
              ...
              const key = `${gridY}_${gridX}_${text}`;
              
              // 将被劫持的内容存入容器
              if (!capturedPoints.has(key)) {
                capturedPoints.set(key, {
                  text: text.trim(),
                  x,
                  y,
                  timestamp: Date.now()
            ...
            // 调用原始 fillText 方法绘制内容，使浏览器正常渲染
            return originalFillText.call(this, text, x, y, maxWidth);
          };
          context.fillText.toString = () => originalFillText.toString();
        }
        // 返回 context 对象
        return context;
      };
      HTMLCanvasElement.prototype.getContext.toString = () => originalGetContext.toString();

      console.log("[Hunter Sandbox] Hijack initialized");
    } catch (err) {
      console.error("[Hunter Sandbox] Hijack failed:", err);
    }
  }

  if (location.href.includes("zhipin.com") || location.href.includes("frame/c-resume")) {
    initHijack();
  }
})();
```
