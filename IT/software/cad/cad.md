...
1. 解压下载好的安装包，解压密码 `www.cadzxw.com`
...
9. 复制 `激活码` 至 `我具有Autodesk提供的激活码` 中，点击 `下一步`
...
- 以下设置只跟cad软件有关，跟样板文件无关

- 各种插件添加至启动组

  - 将插件复制到cad安装目录下的“support”文件夹内
...
1. 打开样板文件，另存为DWG格式，重命名
2. 复制效果平面图到画布，1:1对齐
3. 绘制墙体、下水管道、地漏、烟风道、门口线、梁、窗、指北针、入户箭头、举架、房间名称、量房信息、标注
4. 打开所有图层，复制原始结构图用以创建墙体拆改图，不要忘记复制墙中线，将墙体拆改图放入布局空间
5. 用同样方法逐一进行绘图
...
- **线**
  ...
  - `ML` 多线，多线编辑：修改–对象–多线，个人命令可添加快捷键MLB
...
- `MI` 镜像
- `RO` 旋转
- `EX` 延伸，`EX空格`，选择延伸边界，空格，选择被延伸对象，空格。（此功能可以按shift键当修剪使用）
- `S` 拉伸
- `EX` 延伸 
- `LEN` 拉长
-  **圆角和修剪**
