...
使用 Obsidian 的 [Tag Wrangler](<obsidian.md#Tag Wrangler>)  插件批量修改标签

## 批量删除 Tag

使用 VS Code  批量删除标签。

- 在 Obsidian 中搜索 “my-tag" 的总数量
- 在 VS Code  中把 `notes` 作为项目打开
- 使用 `Ctrl + Shift + F` 打开全局搜索
- 搜索 `  - my-tag` （注意前面有两个空格，因为元数据源码的标签就有两个空格）
  - 如果 VS Code 搜索到总数量和 Obsidian 相等，直接替换为空内容即可删除。
  - 如果不相等，可能是匹配到了正文中的内容，要找到这些内容，防止误删。
