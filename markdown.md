...
- 使用双重反引号 <code>\`\`</code> 转义带有行内代码的内容，为保证生效，最好前后都加一个空格。
...
[Alert](https://docs.github.com/zh/get-started/writing-on-github/getting-started-with-writing-and-formatting-on-github/basic-writing-and-formatting-syntax#alerts) (或 Callout) 属于 Markdown 的**扩展语法**。
...
> [!NOTE]
补充说明：用户快速浏览时也需了解的实用信息。

> [!TIP]
实用技巧：帮助把事情做得更好、更简便的建议。

> [!IMPORTANT]
重要：达成目标必须知晓的关键信息。

> [!WARNING]
警告：需立即注意，避免出现问题的紧急提示。

> [!CAUTION]
谨慎：提示某些操作存在风险或可能导致不良后果。
...
- 代码块内部不能有连续空行，否则会把行首的 `#` 误认为是标题。
- 不会格式化*内联 HTML 代码*的缩进。
